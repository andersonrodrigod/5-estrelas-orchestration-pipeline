...
pasta_saida = Path('saida_resumo_negativas') / 'exec_03_classificacao'
...
def aplicar_regras_com_prefixo_false(df_base, grupos, nomes_classificacao, total_regras_false):
    ...
    for ordem, grupo_original in enumerate(grupos, start=1):
        ...
        if grupo['filtro_local']:
            filtro = filtro & df['LOCAL'].str.contains(
                grupo['filtro_local'], case=False, na=False, regex=True
            )

        if grupo['filtro_local_diferente']:
            filtro = filtro & ~df['LOCAL'].str.contains(
                grupo['filtro_local_diferente'], case=False, na=False, regex=True
            )

        if grupo['filtro_especialidade']:
            filtro = filtro & df['ESPECIALIDADE'].str.contains(
                grupo['filtro_especialidade'], case=False, na=False, regex=True
            )

        if grupo['filtro_especialidade_diferente']:
            filtro = filtro & ~df['ESPECIALIDADE'].str.contains(
                grupo['filtro_especialidade_diferente'], case=False, na=False, regex=True
            )
    ...
    sobrescritas_ultima_regra = [
        item for item in sobrescritas if item['ORDEM_REGRA_NOVA'] == total_regras_false
    ]
    return sobrescritas_ultima_regra


print('Iniciando auditoria incremental de sobrescritas da execucao 03 negativas...')
...
for total_regras_false in range(1, len(grupos) + 1):
    ...
    arquivo_sobrescritas = (
        pasta_saida
        / (
            f'exec_03_classificacao_sobrescritas_ate_regra_{total_regras_false:03d}'
