...
def registrar_sobrescritas(df_base, filtro_aplicacao, regra):
    ...
    resumo_sobrescritas = (
        ...
        .assign(
            ...
            COLUNA_FILTRO_1=formatar_valor_regra(regra.get('coluna_1')),
            COMPARADOR_1=formatar_valor_regra(regra.get('comparador_1')),
            VALOR_1=formatar_valor_regra(regra.get('valor_1')),
            COLUNA_FILTRO_2=formatar_valor_regra(regra.get('coluna_2')),
            COMPARADOR_2=formatar_valor_regra(regra.get('comparador_2')),
            VALOR_2=formatar_valor_regra(regra.get('valor_2')),
            COLUNA_FILTRO_3=formatar_valor_regra(regra.get('coluna_3')),
            COMPARADOR_3=formatar_valor_regra(regra.get('comparador_3')),
            VALOR_3=formatar_valor_regra(regra.get('valor_3')),
        )
        .groupby([
    ...
    return resumo_sobrescritas.to_dict('records')
...
def executar():
    ...
    for _, regra in df_regras.iterrows():
        ...
        sobrescritas = registrar_sobrescritas(df, filtro_aplicacao, regra)

        nome_lista_slug = slugificar(regra['nome_lista'])
        grupo_slug = slugificar(regra['chave_grupo'])
        arquivo_sobrescritas = (
            pasta_saida
            / (
                f'exec_04_classificacao_sobrescritas_regra_{int(regra["ordem"]):03d}'
