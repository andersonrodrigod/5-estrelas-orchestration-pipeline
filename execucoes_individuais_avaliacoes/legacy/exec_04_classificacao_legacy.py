...
arquivo_saida = Path('data_exec/avaliacoes/04_base_com_classificacao_legacy.csv')
...
arquivo_sobrescritas_csv = pasta_resumo / 'exec_04_classificacao_legacy_sobrescritas.csv'
arquivo_nao_classificados_detalhado_csv = pasta_resumo / 'exec_04_classificacao_legacy_nao_classificados_detalhado.csv'
...
def descrever_regra(grupo):
    ...
    if normalizar_flag(grupo.get('aplicar_somente_vazios')):
...
print('Iniciando execucao 04 legacy - classificacao...')
...
for ordem, grupo in enumerate(grupos, start=1):
    ...
    aplicar_somente_vazios = normalizar_flag(grupo.get('aplicar_somente_vazios'))
...
resumo = {
    'execucao': 'exec_04_classificacao_legacy',
...
linhas_txt = [
    'RESUMO DA EXECUCAO 04 LEGACY - CLASSIFICACAO',
    '',
    'ARQUIVOS:',
