...
ARQUIVO_ENTRADA = Path('data_exec/avaliacoes/06_base_com_operadora.csv')
ARQUIVO_SAIDA = Path('data_exec/avaliacoes/07_base_com_meta.csv')
...
PASTA_RESUMO = Path('auditoria') / 'saida_resumo_avaliacoes' / 'exec_07_meta'
...
def validar_mapeamento_meta(df_saida, mapa_meta):
    ...
    classificacao_preenchida = chave_meta != ''
    meta_esperada_vazia = meta_esperada.isna() | (
        meta_esperada.astype('string').str.strip() == ''
    )
    sem_meta_cadastrada = classificacao_preenchida & meta_esperada_vazia

    if sem_meta_cadastrada.any():
        resumo_sem_meta = (
            df_saida.loc[sem_meta_cadastrada, ['CLASSIFICACAO']]
            .fillna('VAZIO')
            .value_counts()
        ...
        for _, linha in resumo_sem_meta.head(LIMITE_EXEMPLOS).iterrows():
            registrar_erro(
                erros,
                f"CLASSIFICACAO '{linha['CLASSIFICACAO']}' sem META cadastrada "
                f"na aba insumos: {int(linha['QUANTIDADE'])} linha(s).",
            )

        if len(resumo_sem_meta) > LIMITE_EXEMPLOS:
            registrar_erro(
                erros,
                f"... mais {len(resumo_sem_meta) - LIMITE_EXEMPLOS} "
                "classificacao(oes) sem META cadastrada.",
            )
